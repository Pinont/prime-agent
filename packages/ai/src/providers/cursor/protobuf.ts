...
/** Ported from oh-my-pi (MIT). isRecord was previously imported from
 * @oh-my-pi/pi-utils; inlined here. */
	...
	switch (kind) {
		...
		case "bytes":
			return scalar(
				...
				(reader) => reader.bytes(),
				(value) => Buffer.from(value).toString("base64"), // Node: Uint8Array.toBase64 absent from repo TS lib
